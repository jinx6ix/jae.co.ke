// app/fr/budget-tours/[slug]/page.tsx
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `${BASE_URL}/fr/budget-tours/${slug}`, languages: buildHreflangAlternates(`/budget-tours/${slug}`) },
    openGraph: { title: tour.title, description: tour.metaDescription, url: `${BASE_URL}/fr/budget-tours/${slug}`, locale: "fr_FR", type: "website", images: [{ url: imageUrl, width: 1200, height: 630, alt: tour.title }] },
  }
}
...
export default async function BudgetTourPage({ params }: Props) {
  ...
  const schema = {
    ...
    url:`${BASE_URL}/fr/budget-tours/${slug}`,
    ...
    <>
      ...
      <nav className="bg-gray-50 py-3 px-4 text-sm" dir="ltr">
        <div className="max-w-7xl mx-auto flex items-center gap-2 text-gray-500">
          ...
          <Link href="/fr/tours" className="hover:text-orange-500">Budget Tours</Link><span>/</span>
      ...
      <div className="max-w-7xl mx-auto px-4 py-12" dir="ltr">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          <div className="lg:col-span-2 space-y-10">
            <div><h2 className="text-2xl font-serif font-bold mb-4">Aperçu</h2><p className="text-gray-700 leading-relaxed">{tour.shortDescription}</p></div>
            {tour.highlights?.length>0&&(
              <div><h2 className="text-2xl font-serif font-bold mb-4">Points forts</h2><ul className="space-y-3">{tour.highlights.map((h: string)=><li key={h} className="flex items-start gap-3 text-gray-700"><span className="text-orange-500">✓</span>{h}</li>)}</ul></div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {tour.included?.length>0&&<div className="bg-green-50 rounded-2xl p-6"><h3 className="font-bold text-green-800 mb-3">✅ Inclus</h3><ul>{tour.included.map((i: string)=><li key={i} className="text-green-700 text-sm">• {i}</li>)}</ul></div>}
              {tour.excluded?.length>0&&<div className="bg-red-50 rounded-2xl p-6"><h3 className="font-bold text-red-800 mb-3">❌ Non inclus</h3><ul>{tour.excluded.map((e: string)=><li key={e} className="text-red-700 text-sm">• {e}</li>)}</ul></div>}
            </div>
            {tour.faqs?.length>0&&(
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24 bg-white rounded-2xl shadow-xl border p-6">
              ...
              <div className="space-y-3 mb-6 text-sm border-t border-b py-4">
                <div className="flex justify-between"><span className="text-gray-500">Durée</span><span>{tour.duration}</span></div>
                <div className="flex justify-between"><span className="text-gray-500">Groupe</span><span>{tour.groupSize}</span></div>
                <div className="flex justify-between"><span className="text-gray-500">Depart</span><span>{tour.departure}</span></div>
              </div>
              <a href={`https://wa.me/254726485228?text=Bonjour%2C%20je%20veux%20réserver%3A%20${encodeURIComponent(tour.title)}`} target="_blank" rel="noopener noreferrer" className="block w-full bg-orange-500 hover:bg-orange-600 text-white text-center font-bold py-4 rounded-xl mb-3">💬 Réserver via WhatsApp</a>
              ...
              <p className="text-center text-xs text-gray-400 mt-4">Réservation flexible · Devis gratuit</p>
            </div>
          </div>
        </div>
      </div>
