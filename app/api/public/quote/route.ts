...
//   Host: mail.jaetravel.co.ke
//   Port: 465
//   Encryption: SSL/TLS
//   Authentication: required
//
// Do not disable TLS certificate verification here. If the certificate
...
async function handleMultiLegQuote(
  ...
  const byLeg = legs.map((leg, idx) => ({
    ...
    clientRatePerPersonSharing:
      legMeta[idx].clientRatePerPersonSharing ?? undefined,
  }));
...
async function sendBookingEmails(args: BookingEmailArgs): Promise<{
  ...
  if (!t) {
    console.error(
      '[quote] SMTP is not configured. Expected SITE_SMTP_HOST, SITE_SMTP_PORT, SITE_SMTP_USER and SITE_SMTP_PASS.',
    );
  ...
  try {
    ...
    console.log(
      `[quote] SMTP connection verified successfully: ${process.env.SITE_SMTP_HOST}:${process.env.SITE_SMTP_PORT || '465'}`,
    );
  } catch (error: any) {
    console.error('[quote] SMTP verification failed:', {
      name: error?.name,
      code: error?.code,
  ...
  const fromAddr =
    process.env.SITE_SMTP_USER?.trim() || 'info@jaetravel.co.ke';

  const fmtDate = (d: Date) => d.toISOString().split('T')[0];

  const fmtMoney = (n: number) =>
    `${args.currency} ${n.toLocaleString(undefined, {
      ...
      (s) =>
        `<tr>
          <td style="padding:8px 0;">
            ...
            ${
              s.county
                ? ` <span style="color:#6b7280;">(${s.county})</span>`
                : ''
            }
          </td>
          <td style="padding:8px 0; text-align:right;">${s.nights}</td>
          <td style="padding:8px 0; text-align:right;">
            ${fmtMoney(s.pricePerNight)}
          </td>
        </tr>`,
    )
    .join('');
  ...
  <div style="max-width:600px; margin:20px auto; background:white; border-radius:16px; overflow:hidden; box-shadow:0 10px 25px rgba(0,0,0,0.08);">
    ...
    <div style="padding:32px 24px;">
      ...
      <p>
        ...
        Our safari consultants will be in touch within
        <strong>24 hours</strong> to finalise your itinerary.
      </p>

      <div style="background:#fffbeb; border:1px solid #fcd34d; border-radius:12px; padding:20px; margin:20px 0;">
        <h2 style="margin:0 0 12px; color:#92400e; font-size:18px; border-bottom:2px solid #fbbf24; padding-bottom:6px;">
          Your Enquiry
        </h2>

        <table style="width:100%; font-size:15px;">
          <tr>
            <td style="padding:6px 0; color:#92400e; font-weight:600;">Reference:</td>
            <td>
          ...
          <tr>
            <td style="padding:6px 0; color:#92400e; font-weight:600;">Dates:</td>
            <td>${fmtDate(args.startDate)} → ${fmtDate(args.endDate)}</td>
          </tr>

          <tr>
            <td style="padding:6px 0; color:#92400e; font-weight:600;">Guests:</td>
            <td>
              ${args.numAdults}
              adult${args.numAdults === 1 ? '' : 's'}
              ${
                args.numChildren
                  ? `, ${args.numChildren} child${
                      args.numChildren === 1 ? '' : 'ren'
                    }`
                  : ''
              }
            </td>
          ...
          <tr>
            <td style="padding:6px 0; color:#92400e; font-weight:600;">Total:</td>
            <td>
              <strong style="color:#dc2626; font-size:17px;">
                ${fmtMoney(args.totalCost)}
              </strong>
            </td>
          </tr>

          <tr>
            <td style="padding:6px 0; color:#92400e; font-weight:600;">Per person:</td>
            <td>${fmtMoney(args.perPersonCost)}</td>
          </tr>
        </table>
      </div>
      ...
      ${
        args.selections.length
          ? `
      <h3 style="margin:24px 0 8px; color:#1f2937;">
        Selected Stays
      </h3>
      ...
      <table style="width:100%; border-collapse:collapse; font-size:14px;">
        <thead>
          <tr style="background:#f3f4f6;">
            ...
            <th style="text-align:right; padding:8px;">Nights</th>
            <th style="text-align:right; padding:8px;">/night</th>
          </tr>
        </thead>
        <tbody>
          ${selectionsRows}
      ...
      <pre style="background:#f9fafb; border:1px solid #e5e7eb; border-radius:8px; padding:12px; font-family:inherit; white-space:pre-wrap; font-size:13px;">${args.notes.replace(
        /[&<>]/g,
        (char) =>
          ({
            '&': '&amp;',
    ...
    <div style="background:#f3f4f6; padding:24px; text-align:center; font-size:13px; color:#6b7280; border-top:1px solid #e5e7eb;">
      <p style="margin:0;">
        © 2025 JaeTravel Expeditions | TTA/0036 | Nairobi, Kenya
      </p>

      <p style="margin:6px 0 0;">
        <a href="tel:+254726485228" style="color:#f97316; text-decoration:none;">
          +254 726 485 228
        </a>
        |
        <a href="mailto:info@jaetravel.co.ke" style="color:#f97316; text-decoration:none;">
...
<body style="margin:0; padding:0; font-family:'Segoe UI',Tahoma,sans-serif; background:#f9fafb; color:#1f2937;">
  ...
  <div style="max-width:600px; margin:20px auto; background:white; border-radius:16px; overflow:hidden;">
    ...
    <div style="padding:32px 24px;">
      ...
      <h2 style="margin:0 0 12px; color:#059669; font-size:20px;">
        Customer
      </h2>
      ...
      <div style="background:#f0fdf4; border:1px solid #bbf7d0; border-radius:12px; padding:20px; margin-bottom:20px;">
        <table style="width:100%; font-size:15px;">
          ...
          <tr>
            <td style="padding:6px 0; color:#166534; font-weight:600; width:120px;">
              Name:
            </td>
            <td><strong>${args.name}</strong></td>
          </tr>
          ...
          <tr>
            ...
            <td>
              <a href="mailto:${args.email}" style="color:#059669;">
                ${args.email}
              </a>
            </td>
          </tr>
          ...
          <tr>
            ...
            <td>
              <a href="tel:${args.phone}" style="color:#059669;">
                ${args.phone}
              </a>
            </td>
          </tr>
      ...
      <div style="background:#f0fdf4; border:1px solid #bbf7d0; border-radius:12px; padding:20px; margin-bottom:20px;">
        <table style="width:100%; font-size:15px;">
          ...
          <tr>
            ...
            <td>
              <code style="background:#d1fae5; padding:4px 8px; border-radius:4px;">
                ${args.bookingRef}
              </code>
            </td>
          </tr>
          ...
          <tr>
            ...
            <td>
              ${args.numAdults}A / ${args.numChildren}C
            </td>
          </tr>
          ...
          <tr>
            ...
            <td>
              <strong style="color:#059669; font-size:17px;">
                ${fmtMoney(args.totalCost)}
              </strong>
            </td>
          </tr>
...
<body style="font-family:'Segoe UI',Tahoma,sans-serif; color:#1f2937;">
  ...
  <table style="border-collapse:collapse; font-size:14px;">
    ...
    <tr>
      ...
      <td><code>${args.bookingRef}</code></td>
    </tr>
    ...
    <tr>
      ...
      <td>${args.numAdults}A / ${args.numChildren}C</td>
    </tr>
    ...
    <tr>
      ...
      <td>
        <strong>${fmtMoney(args.totalCost)}</strong>
      </td>
    </tr>
  ...
  <p style="font-size:13px; color:#6b7280;">
    Automated notification from the public quote/booking endpoint.
  </p>
  ...
  const sends = [
    {
      key: 'client' as const,
      msg: {
        from: `"JaeTravel Expeditions" <${fromAddr}>`,
        to: args.email,
        replyTo: fromAddr,
        subject: `Booking Enquiry #${args.bookingRef} – Received!`,
        html: clientHtml,
      },
    },
    {
      key: 'info' as const,
      msg: {
        from: `"JaeTravel Bookings" <${fromAddr}>`,
        to: 'info@jaetravel.co.ke',
        replyTo: args.email,
        subject: `New Booking #${args.bookingRef} – ${args.name}`,
        html: infoHtml,
      },
    },
    {
      ...
      msg: {
        from: `"JaeTravel Bookings" <${fromAddr}>`,
        to: 'it@jaetravel.co.ke',
        replyTo: args.email,
        subject: `[IT] New booking ${args.bookingRef} – ${fmtMoney(
          args.totalCost,
        )}`,
        html: itHtml,
      },
  ...
  const results = await Promise.allSettled(
    sends.map((s) => t.sendMail(s.msg)),
  );

  const status = {
    client: 'skipped' as 'sent' | 'failed' | 'skipped',
    info: 'skipped' as 'sent' | 'failed' | 'skipped',
    it: 'skipped' as 'sent' | 'failed' | 'skipped',
  };

  results.forEach((result, index) => {
    const key = sends[index].key;

    if (result.status === 'fulfilled') {
      status[key] = 'sent';

      console.log(
        `[quote] ${key} email sent successfully for ${args.bookingRef}`,
      );
    } else {
      status[key] = 'failed';
      ...
      console.error(`[quote] Failed to send ${key} email for ${args.bookingRef}:`, {
        name: error?.name,
        code: error?.code,
        command: error?.command,
        response: error?.response,
  ...
  console.log(
    `[quote] Email status for ${args.bookingRef}: client=${status.client} info=${status.info} it=${status.it}`,
  );
