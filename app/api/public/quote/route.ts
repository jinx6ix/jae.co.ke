...
async function persistAndRespond(args: {
  ...
  const customerWhatsApp = body.phone ? `https://wa.me/${body.phone.replace(/[^0-9]/g, '').replace(/^0/, '254')}` : undefined;
  ...
  const emailStatus = await sendBookingEmails({
    ...
    service: {
      name: body.tourPackageId ? 'Custom itinerary (from template)' : 'Custom itinerary (built on website)',
      startDate: fmtDate(start),
      travelers: numAdults + numChildren,
      totalPrice: totalCost,
      specialRequirements: buildNotes(body),
    },
    adminWhatsApp,
    customerWhatsApp,
  });
...
function fmtDate(start: Date): string {
