// app/de/budget-tours/[slug]/page.tsx
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `${BASE_URL}/de/budget-tours/${slug}`, languages: buildHreflangAlternates(`/budget-tours/${slug}`) },
    openGraph: { title: tour.title, description: tour.metaDescription, url: `${BASE_URL}/de/budget-tours/${slug}`, locale: "de_DE", type: "website", images: [{ url: imageUrl, width: 1200, height: 630, alt: tour.title }] },
  }
}
...
export default async function BudgetTourPage({ params }: Props) {
  ...
  const schema = {
    ...
    url:`${BASE_URL}/de/budget-tours/${slug}`,
    ...
    <>
      ...
      <nav className="bg-gray-50 py-3 px-4 text-sm" dir="ltr">
        <div className="max-w-7xl mx-auto flex items-center gap-2 text-gray-500">
          ...
          <Link href="/de/tours" className="hover:text-orange-500">Budget Touren</Link><span>/</span>
      ...
      <div className="max-w-7xl mx-auto px-4 py-12" dir="ltr">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          <div className="lg:col-span-2 space-y-10">
            <div><h2 className="text-2xl font-serif font-bold mb-4">Überblick</h2><p className="text-gray-700 leading-relaxed">{tour.shortDescription}</p></div>
            {tour.highlights?.length>0&&(
              <div><h2 className="text-2xl font-serif font-bold mb-4">Highlights</h2><ul className="space-y-3">{tour.highlights.map((h: string)=><li key={h} className="flex items-start gap-3 text-gray-700"><span className="text-orange-500">✓</span>{h}</li>)}</ul></div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {tour.included?.length>0&&<div className="bg-green-50 rounded-2xl p-6"><h3 className="font-bold text-green-800 mb-3">✅ Inklusive</h3><ul>{tour.included.map((i: string)=><li key={i} className="text-green-700 text-sm">• {i}</li>)}</ul></div>}
              {tour.excluded?.length>0&&<div className="bg-red-50 rounded-2xl p-6"><h3 className="font-bold text-red-800 mb-3">❌ Exklusive</h3><ul>{tour.excluded.map((e: string)=><li key={e} className="text-red-700 text-sm">• {e}</li>)}</ul></div>}
            </div>
            {tour.faqs?.length>0&&(
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24 bg-white rounded-2xl shadow-xl border p-6">
              ...
              <div className="space-y-3 mb-6 text-sm border-t border-b py-4">
                <div className="flex justify-between"><span className="text-gray-500">Dauer</span><span>{tour.duration}</span></div>
                <div className="flex justify-between"><span className="text-gray-500">Gruppe</span><span>{tour.groupSize}</span></div>
                <div className="flex justify-between"><span className="text-gray-500">Depart</span><span>{tour.departure}</span></div>
              </div>
              <a href={`https://wa.me/254726485228?text=Hallo%2C%20ich%20möchte%20buchen%3A%20${encodeURIComponent(tour.title)}`} target="_blank" rel="noopener noreferrer" className="block w-full bg-orange-500 hover:bg-orange-600 text-white text-center font-bold py-4 rounded-xl mb-3">💬 Über WhatsApp buchen</a>
              <a href="mailto:info@jaetravel.co.ke" className="block w-full border border-orange-500 text-orange-500 text-center font-semibold py-3 rounded-xl text-sm">✉️ E-Mail</a>
              <p className="text-center text-xs text-gray-400 mt-4">Flexible Buchung · Kostenloses Angebot</p>
            </div>
          </div>
        </div>
      </div>
