...
async function loadProduct(slug: string) {
  const cms = await getCmsProductBySlug(slug)
  const staticProduct = products.find((p) => p.slug === slug)
  if (cms) {
    return {
      ...(staticProduct ?? {} as any),
      name: cms.title,
      description: cms.description || staticProduct?.description || '',
      details: cms.details ? '' : (staticProduct as any)?.details || '', // richText rendered separately
      price: cms.price ?? staticProduct?.price ?? 0,
      duration: cms.duration || staticProduct?.duration || '',
      metaTitle: cms.meta?.title || staticProduct?.metaTitle || cms.title,
      metaDescription: cms.meta?.description || staticProduct?.metaDescription || '',
      // Pass through richText as `cmsDetails` so the page can render it
      cmsDetails: cms.details,
    } as any
  }
  return staticProduct as any
}
