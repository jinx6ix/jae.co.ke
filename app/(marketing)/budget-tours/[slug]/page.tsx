...
import { getCmsBudgetTourBySlug } from "@/lib/cms-budget-tours";
...
async function loadTour(slug: string) {
  const cms = await getCmsBudgetTourBySlug(slug)
  const staticTour = getTourBySlug(slug)
  if (cms) {
    return {
      ...(staticTour ?? {}),
      title: cms.title,
      description: cms.description || staticTour?.description || '',
      shortDescription: cms.shortDescription || staticTour?.shortDescription || '',
      price: cms.price ?? staticTour?.price ?? 0,
      originalPrice: cms.originalPrice ?? staticTour?.originalPrice,
      duration: cms.duration || staticTour?.duration || '',
      groupSize: cms.groupSize || staticTour?.groupSize || '',
      country: cms.country || staticTour?.country || '',
      rating: cms.rating ?? staticTour?.rating ?? 0,
      reviewCount: cms.reviewCount ?? staticTour?.reviewCount ?? 0,
      highlights: cms.highlights?.map((h: any) => h.text).filter(Boolean) || staticTour?.highlights || [],
      included: cms.included?.map((h: any) => h.text).filter(Boolean) || staticTour?.included || [],
      excluded: cms.excluded?.map((h: any) => h.text).filter(Boolean) || staticTour?.excluded || [],
      itinerary: cms.itinerary?.map((it: any) => ({ day: it.day, title: it.title, content: it.content, meals: (it as any).meals })) || staticTour?.itinerary || [],
      faqs: cms.faqs?.map((f: any) => ({ question: f.question, answer: f.answer })) || staticTour?.faqs || [],
      metaDescription: cms.meta?.description || staticTour?.metaDescription || '',
    }
  }
  return staticTour
}
