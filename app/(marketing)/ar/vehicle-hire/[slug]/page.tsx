...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${vehicle.metaTitle} | جيه تريفل`,
    description: vehicle.metaDescription,
    keywords: vehicle.keywords,
    alternates: { canonical: `${BASE_URL}/ar/vehicle-hire/${slug}`, languages: buildHreflangAlternates(`/vehicle-hire/${slug}`) },
    openGraph: { title: vehicle.metaTitle, description: vehicle.metaDescription, url: `${BASE_URL}/ar/vehicle-hire/${slug}`, locale: "ar_AE", type: "website", images: [{ url: imageUrl, width: 1200, height: 630, alt: vehicle.name }] },
  }
}
...
export default async function VehicleHirePage({ params }: Props) {
  ...
  const schema = {
    ...
    url:`${BASE_URL}/ar/vehicle-hire/${slug}`,
    ...
    <>
      ...
      <nav className="bg-gray-50 py-3 px-4 text-sm" dir="rtl">
        <div className="max-w-7xl mx-auto flex items-center gap-2 text-gray-500">
          ...
          <Link href="/ar/vehicle-hire" className="hover:text-orange-500">تأجير المركبات</Link><span>/</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          <div className="lg:col-span-2 space-y-10">
            ...
            {vehicle.features?.length>0&&(
              <div><h2 className="text-2xl font-serif font-bold mb-4">المميزات</h2>
            ...
            <div><h2 className="text-2xl font-serif font-bold mb-4">المواصفات</h2>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24 bg-white rounded-2xl shadow-xl border p-6">
              <div className="text-center mb-6">
                ...
                <p className="text-gray-500 text-sm">في اليوم</p>
              </div>
              ...
              <a href={`https://wa.me/254726485228?text=مرحباً%2C%20أريد%20استئجار%20المركبة%3A%20${encodeURIComponent(vehicle.name)}`} target="_blank" rel="noopener noreferrer" className="block w-full bg-orange-500 hover:bg-orange-600 text-white text-center font-bold py-4 rounded-xl mb-3">💬 احجز عبر واتساب</a>
              <a href="mailto:info@jaetravel.co.ke" className="block w-full border border-orange-500 text-orange-500 text-center font-semibold py-3 rounded-xl text-sm">✉️ بريد</a>
            </div>
          </div>
        </div>
      </div>
