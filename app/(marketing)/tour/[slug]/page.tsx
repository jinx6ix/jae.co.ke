...
export default async function TourPage(props: TourPageProps) {
  ...
  const staticTour = tours.find((t) => t.slug === slug)
  const cmsTour = await getCmsTourBySlug(slug)

  if (!staticTour && !cmsTour) {
    notFound()
  }

  // Static data is the source of truth for fields CMS doesn't track
  // (itinerary, included, excluded, duration, etc.). CMS marketing
  // overrides win for fields it does track (title, shortDescription,
  // highlights, gallery, meta) when a tour doc is published.
  const tour: Tour = staticTour
    ? {
        ...staticTour,
        title: cmsTour?.title || staticTour.title,
        shortDescription:
          cmsTour?.shortDescription || staticTour.shortDescription,
        description:
          staticTour.description /* description is rich-text in CMS */,
        highlights:
          cmsTour?.highlights && cmsTour.highlights.length > 0
            ? cmsTour.highlights
                .map((h) => h?.text)
                .filter((t): t is string => typeof t === 'string')
            : staticTour.highlights,
        metaTitle:
          cmsTour?.meta?.title || staticTour.metaTitle || staticTour.title,
        metaDescription:
          cmsTour?.meta?.description ||
          staticTour.metaDescription ||
          staticTour.description,
      }
    : // Pure-CMS tour (no static fallback). Synthesize a minimal Tour
      // record from the CMS doc so the JSX below doesn't crash.
      ({
        id: cmsTour!.id,
        slug: cmsTour!.slug ?? slug,
        title: cmsTour!.title,
        location: '',
        checkInText: '',
        tags: null,
        shortDescription: cmsTour!.shortDescription ?? '',
        image: '',
        duration: '',
        highlights:
          cmsTour!.highlights
        ...
        groupSize: '',
        category: '',
        metaTitle: cmsTour!.meta?.title ?? cmsTour!.title,
        metaDescription: cmsTour!.meta?.description ?? '',
        url: `/tour/${slug}`,
        description: cmsTour!.shortDescription ?? '',
        price: 0,
        currency: 'USD',
        rating: 0,
        reviewCount: 0,
