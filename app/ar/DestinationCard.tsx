// app/ar/destinations/DestinationCard.tsx - نسخة عربية (RTL)
'use client'
...
export default function DestinationCard({ destination }: { destination: Destination }) {
  return (
    <Link
      href={`/ar/destination/${destination.slug}`}
      className="group relative aspect-[16/10] overflow-hidden rounded-2xl shadow-lg transition-all hover:shadow-xl"
      dir="rtl"
    >
      <Image
        src={destination.heroImage}
        alt={`${destination.name} - وجهة سفاري في شرق أفريقia تشتهر بـ ${destination.bestFor.join("، ")}`}
        fill
      ...
      <div className="absolute bottom-0 left-0 right-0 p-8 text-white">
        ...
        <div className="flex items-center gap-2 font-medium text-white">
          <span>استكشف {destination.name}</span>
          <ArrowRight className="h-5 w-5 transition-transform group-hover:-translate-x-1" /> {/* عكس اتجاه السهم في RTL */}
        </div>
      </div>
    </Link>
  )
