// app/ar/destinations/DestinationCard.tsx
'use client'
...
export default function DestinationCard({ destination }: { destination: Destination }) {
  return (
    <Link
      href={`/ar/destinations/${destination.slug}`}
      className="group relative aspect-[16/10] overflow-hidden rounded-2xl shadow-lg transition-all hover:shadow-xl"
    >
      <Image
        src={destination.heroImage}
        alt={`${destination.name} - وجهة سفاري في شرق أفريقيا تتميز بـ ${destination.bestFor.join("، ")}`}
        fill
        sizes="(max-width: 768px) 100vw, 50vw"
        className="object-cover transition-transform duration-700 group-hover:scale-105"
        priority
      />
      <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent" dir="rtl" />
