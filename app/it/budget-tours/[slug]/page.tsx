// app/it/budget-tours/[slug]/page.tsx
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `${BASE_URL}/it/budget-tours/${slug}`, languages: buildHreflangAlternates(`/budget-tours/${slug}`) },
    openGraph: { title: tour.title, description: tour.metaDescription, url: `${BASE_URL}/it/budget-tours/${slug}`, locale: "it_IT", type: "website", images: [{ url: imageUrl, width: 1200, height: 630, alt: tour.title }] },
  }
}
...
export default async function BudgetTourPage({ params }: Props) {
  ...
  const schema = {
    ...
    url:`${BASE_URL}/it/budget-tours/${slug}`,
    ...
    provider:{"@type":"TravelAgency","@id":`${BASE_URL}/#organization`,name:"JaeTravel Spedizioni"},
  }
    ...
    <>
      ...
      <nav className="bg-gray-50 py-3 px-4 text-sm" dir="ltr">
        <div className="max-w-7xl mx-auto flex items-center gap-2 text-gray-500">
          <Link href="/it" className="hover:text-orange-500">Home</Link><span>/</span>
          <Link href="/it/tours" className="hover:text-orange-500">Tour Budget</Link><span>/</span>
      ...
      <div className="max-w-7xl mx-auto px-4 py-12" dir="ltr">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          <div className="lg:col-span-2 space-y-10">
            <div><h2 className="text-2xl font-serif font-bold mb-4">Panoramica</h2><p className="text-gray-700 leading-relaxed">{tour.shortDescription}</p></div>
            {tour.highlights?.length>0&&(
              <div><h2 className="text-2xl font-serif font-bold mb-4">Punti Salienti</h2><ul className="space-y-3">{tour.highlights.map((h: string)=><li key={h} className="flex items-start gap-3 text-gray-700"><span className="text-orange-500">✓</span>{h}</li>)}</ul></div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {tour.included?.length>0&&<div className="bg-green-50 rounded-2xl p-6"><h3 className="font-bold text-green-800 mb-3">✅ Incluso</h3><ul>{tour.included.map((i: string)=><li key={i} className="text-green-700 text-sm">• {i}</li>)}</ul></div>}
              {tour.excluded?.length>0&&<div className="bg-red-50 rounded-2xl p-6"><h3 className="font-bold text-red-800 mb-3">❌ Escluso</h3><ul>{tour.excluded.map((e: string)=><li key={e} className="text-red-700 text-sm">• {e}</li>)}</ul></div>}
            </div>
            {tour.faqs?.length>0&&(
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24 bg-white rounded-2xl shadow-xl border p-6">
              <div className="text-center mb-6">
                ...
                <p className="text-gray-500 text-sm">a persona</p>
              </div>
              ...
              <a href={`https://wa.me/254726485228?text=Ciao%2C%20voglio%20prenotare%3A%20${encodeURIComponent(tour.title)}`} target="_blank" rel="noopener noreferrer" className="block w-full bg-orange-500 hover:bg-orange-600 text-white text-center font-bold py-4 rounded-xl mb-3">💬 Prenota via WhatsApp</a>
              ...
              <p className="text-center text-xs text-gray-400 mt-4">Prenotazione flessibile · Preventivo gratuito</p>
            </div>
          </div>
        </div>
      </div>
